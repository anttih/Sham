...
when asserting. To check if `$stub` object was called once with `foo()` you
would do this:

    $stub = new Sham_Mock();
    $stub->calls('foo')->once();

`once()` returns a boolean.

To check if `foo()` was called once with 'first' as the only parameter:

    $stub->calls('foo', 'first')->once();

To check if `foo()` was called with anything as the first param and `second` as
the second param:

    $stub->calls('foo', Sham::any(), 'second')->once();
...
return value of your choosing for a method call.

    $stub->method->returns('foo');
    $stub->method(); // 'foo'

You can also make calls run code when they are invoked. These side effects can
